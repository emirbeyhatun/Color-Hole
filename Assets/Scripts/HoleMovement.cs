...
public class HoleMovement : MonoBehaviour
{
	...
	private bool firstTargetPosIsReached = false;
	...
	void Update ()
	{
		...
		if (!Game.isGameover && Game.isMoving  && !Game.transaction) {
			//Move hole center
			MoveHole ();
		...
		/*if(Game.transaction)
		{
			if(firstTargetPosIsReached == false)
			{
				MoveHole (targetPoints[0].position);
				//Update hole vertices
				UpdateHoleVerticesPosition ();

				if(Vector3.Distance(holeCenter.position, new Vector3(targetPoints[0].position.x ,holeCenter.position.y, targetPoints[0].position.z)) < 0.2f)
				{
					firstTargetPosIsReached = true;
				}
			}
			else
			{
				MoveHole (targetPoints[1].position);
				//Update hole vertices
				UpdateHoleVerticesPosition ();

				if(Vector3.Distance(holeCenter.position, new Vector3(targetPoints[1].position.x ,holeCenter.position.y, targetPoints[1].position.z))< 0.2f)
				{
					firstTargetPosIsReached = true;
					Game.transaction = false;
					Magnet.Instance.EnableHole();
				}
			}
		}*/
	...
	void MoveHole (Vector3 trgPos)
	{
		//lerp (smooth) movement
		touch = Vector3.Lerp (
			holeCenter.position, 
			new Vector3 (trgPos.x, holeCenter.position.y, trgPos.z), //move hole on x & z 
			moveSpeed * Time.deltaTime
		);
